        ...
        for i = 0, province_list:num_items() -1 do
            ...
            if this_province:has_effect_bundle("tp_amenmesse_command_dummy","phar_main_amenmesse") then
                ...
                if provinces_with_gold[this_prov_name] then
                    out("tp_gold: adding pooled resource")
                    local current_deposit = this_province:depletable_resource_remaining_amount_no_faction("troy_gold")
                    local value = current_deposit + 3200 --1600 is about 5 turns of 1 settlment production with 56% increase rate.
                    cm:province_set_pooled_resource(this_province:name(),"troy_gold",value) 
                else
                    out("tp_gold: command done in invalid province. can we try and reset the timers.")
