...
function sword_of_khaine:level_up(awarded_via_dilemma)
     if (self.owner.lv + 1) <= #self.key_index then
          ...
          if not character then
               script_error("ERROR: sword_of_khaine:level_up() called but supplied character cqi: " ..
               self.owner.character_cqi .. " has not returned a valid character interface");
          end
          local sword_owner_faction = cm:get_faction(self.owner.faction);

          -- Remove current lvl sword from currently stored character_cqi
          self:remove_sword_effect_bundle();
          out("Removing_sword with level [" ..
          self.key_index[self.owner.lv] .. "] from character with cqi [" .. self.owner.character_cqi .. "]");
...
--Jess bookmark. self.owner.character_cqi
function sword_of_khaine:is_character_valid_to_equip_sword(character)
     local mfc = character:Call("MilitaryForceContext")
     local is_patrol_army = mfc:Call("ForceType") == "PATROL_ARMY"
     out ("Lycia's no Sword of Khaine for Hertz Province Patrols. Sword of Khaine character validationn check. is_patrol_army is " .. tostring(is_patrol_army) .. ".")
     return not character:is_null_interface() and cm:char_is_general_with_army(character) and
         not character:military_force():is_armed_citizenry() and
         not character:character_subtype("wh2_main_def_black_ark") and
         not character:character_subtype("wh3_main_dae_daemon_prince") and not is_patrol_army;
end;
...
function sword_of_khaine:transfer_sword_after_battle(target_character)
     if target_character and self:is_character_valid_to_equip_sword(target_character) then
          ...
          if faction:is_human() then
               cm:trigger_dilemma_with_targets(faction:command_queue_index(), self.event_index.sword_picking, 0, 0, cqi,
                    0, 0, 0);
...
function sword_of_khaine:add_listeners()
                         ...
                         if current_faction:is_human() then
                              if self.owner.character_cqi then
                                   ...
                                   elseif self.owner.cd == 0 and self.owner.lv + 1 <= #self.key_index then
                                        cm:trigger_dilemma_with_targets(current_faction:command_queue_index(),
                                             self.event_index.sword_level_dilemma_index[self.owner.lv], 0, 0,
                                             self.owner.character_cqi, 0, 0, 0);
                                   end;
                              end;
                              -- deal with AI
                              ...
                              if attacker_fm and not attacker_fm:character_details():is_null_interface() then
                                   ...
                                   for i2 = 1, #self.key_index do
                                        ...
                                        if attacker_fm:character_details():has_ancillary(ancillary) or self:ancillary_unequipped_check(faction, ancillary) then
                                             local defender_char_cqi, mf_cqi, faction_name = cm
                                             :pending_battle_cache_get_defender(1);

                                             out(
                                             "Sword of Khaine: The previous owner has been defeated in battle, so we'll check to see if " ..
                                             faction_name .. " can steal the sword")
                              ...
                              if defender_fm and not defender_fm:character_details():is_null_interface() then
                                   ...
                                   for i2 = 1, #self.key_index do
                                        ...
                                        if defender_details:has_ancillary(ancillary) or self:ancillary_unequipped_check(faction, ancillary) then
                                             local attacker_char_cqi, mf_cqi, faction_name = cm
                                             :pending_battle_cache_get_attacker(1);
          ...
          function()
               if not camera_is_moving then
                    ...
                    if self.owner.character_cqi then
                         ...
                         if character then
                              cm:scroll_camera_from_current(false, 3,
                                   { character:display_position_x(), character:display_position_y(), 14.7, 0, 12 });
                    ...
                    elseif self.owner.faction == faction:name() and self.owner.ready and self:find_candidate_for_faction(faction) then
                         ...
                         cm:scroll_camera_from_current(false, 3,
                              { settlement:display_position_x(), settlement:display_position_y(), 14.7, 0, 12 });
                         camera_is_moving = true;
                         cm:callback(function() camera_is_moving = false end, 3);
                    end;
               end;
     ...
     core:add_listener(
          ...
          function(context)
               ...
               return building:superchain() == "wh2_main_sch_special_shrine_of_khaine" and
               self:faction_is_elven(building:faction());
          end,
          function(context)
               if not self.owner.character_cqi then
                    self:return_sword_to_shrine();
...
cm:add_loading_game_callback(
     function(context)
          if cm:is_new_game() == false then
               ...
               sword_of_khaine.vfx_cache = cm:load_named_value("sword_of_khaine_vfx_cache", sword_of_khaine.vfx_cache,
                    context);
               sword_of_khaine.owner_cache = cm:load_named_value("sword_of_khaine_owner_cache",
                    sword_of_khaine.owner_cache, context);
          end
     end
);
