          ...
          if character and self:character_has_valid_faction_in_campaign(character) then
               ...
               if has_starting_owner == false then --Remember to add the spawn_hero_for_ai() function to the end of function for any new unlock condition type that you may add.
                    ...
                    elseif current_character.starting_mission_keys then
                         ...
                         script_error("ERROR: Attempted to start unlock listeners for legendary hero [" ..
                         character.subtype .. "] but could not find appropriate data!")
                    end
                         ...
                         function(context)
                              if character_info.has_spawned == false then
                                   ...
                                   if trigger_mission then
                                        ...
                                        local mission_key = self:get_mission_key(character_info.starting_mission_keys,
                                             faction_name)
                                        if is_string(mission_key) then
                                             ...
                                             script_error(
                                             "Legendary Hero spawning script returned >1 or 0 missions for a character when setting up listeners, which would otherwise crash the game")
                                        end
                                   end
                              end
                         end,
     ...
     if not priority_faction_found then
          for i = 1, #character_info.allowed_factions do
               ...
               if faction and faction:is_human() then
                    ...
                    character_info.factions_involved[faction_name] = true
                    quest_hero_unlock_human_count = quest_hero_unlock_human_count + 1
                    self:setup_quest_mission_completed_spawn_hero_listener(character, faction_name,
                         character_mission_success)
               end
          end
     end
...
function character_unlocking:ai_unlock_by_turn(character)
     ...
     core:add_listener(
          ...
          "WorldStartRound",
          function(context)
               return cm:turn_number() >= character_info.ai_unlock_turn and
          ...
          function(context)
               ...
               if character_info.priority_ai_faction ~= nil then
                    if faction_name == character_info.priority_ai_faction then
                         ...
                         return character_interface:rank() >= character_info.ai_unlock_rank and
                         self:character_has_valid_faction_in_campaign(character) and character_info.has_spawned == false
                    end
               end
               ...
               if not has_priority_ai_faciton then
                    for i = 1, #character_info.allowed_factions do
                         if faction_name == character_info.allowed_factions[i] then
                              return character_interface:rank() >= character_info.ai_unlock_rank and
                              self:character_has_valid_faction_in_campaign(character) and
                              character_info.has_spawned == false
                         end
                    end
               end
          end,
...
function character_unlocking:add_dilemma_payload_listener(character)
     ...
     core:add_listener(
          ...
          function(context)
               ...
               if not character_info.spawn_hero_dilemma_choice or self:is_match_key_from_list(choice, character_info.spawn_hero_dilemma_choice) then -- if a choice isn't specifically listed, spawn with any choice
                    self:spawn_hero(context:faction():name(), character,
     ...
     core:add_listener(
          ...
          "MissionSucceeded",
          function(context)
               return self:is_match_key_from_list(context:mission():mission_record_key(),
                    character_info.missions_to_trigger_dilemma)
          end,
          function(context)
     ...
     if not priority_faction_found then
          for i = 1, #character_info.allowed_factions do
               ...
               if faction and faction:is_human() then
                    ...
                    character_info.factions_involved[faction_name] = true
                    building_unlock_human_faction_counter = building_unlock_human_faction_counter + 1
                    self:setup_building_completed_spawn_hero_listener(character, faction_name,
                         character_building_complete)
               end
          end
     end
...
function character_unlocking:spawn_hero(faction_name, character, spawn_character_cqi)
     ...
     if character_info.has_spawned == false then
          ...
          core:add_listener(
               ...
               function(context)
                    character_unlocking:unique_agent_listener(context, faction, character_info, character_info.subtype,
                         character_info.ancillaries)
               end,
               false
          )
          ...
          if character_info.additional_subtype ~= nil then
               ...
               core:add_listener(
                    ...
                    "UniqueAgentSpawned",
                    function(context)
                         return context:unique_agent_details():character():character_subtype(character_info
                         .additional_subtype)
                    end,
                    function(context)
                         character_unlocking:unique_agent_listener(context, faction, character_info,
                              character_info.additional_subtype, character_info.additional_ancillaries)
                    end,
                    false
               )
          end
          ...
          if spawn_character_cqi then
               ...
               elseif faction:has_home_region() then
                    cm:spawn_unique_agent_at_region(faction:command_queue_index(), character_info.subtype,
                         faction:home_region():cqi(), true)

                    if character_info.additional_subtype ~= nil then
                         cm:spawn_unique_agent_at_region(faction:command_queue_index(), character_info
                         .additional_subtype, faction:home_region():cqi(), true)
                    end
               else
                    ...
                    cm:callback(
                         function()
                              if not cm:model():pending_battle():is_active() then
                                   cm:scroll_camera_from_current(true, 1.5,
                                        { agent:display_position_x(), agent:display_position_y(), 6, 0, 6 });
                              end
                         end,
                         0.4
                    )
                    ...
                    cm:callback(
                         function()
                              if not cm:model():pending_battle():is_active() then
                                   ...
                                   cm:scroll_camera_from_current(true, 1.5,
                                        { character:display_position_x(), character:display_position_y(), 6, 0, 6 });
                              end
                         end,
                         0.4
                    )
...
function character_unlocking:get_allowed_factions_list(character_data)
     ...
     if character_data.allowed_cultures then
          ...
          for _, v in ipairs(cultures_list) do
               local culture_to_allowed_factions = self:get_allowed_factions_for_culture(v,
...
function character_unlocking:cancel_missions_for_other_players(completing_faction, character,
                                                               character_mission_success_listener)
     local character_info = self.character_data[character]
     local campaign_name = cm:get_campaign_name()
     for i = 1, #character_info.allowed_factions do
...
function character_unlocking:setup_quest_mission_completed_spawn_hero_listener(character, faction_name,
                                                                               character_mission_success)
     local character_info = self.character_data[character]
...
function character_unlocking:setup_building_completed_spawn_hero_listener(character, faction_name,
                                                                          character_building_complete)
     local character_info = self.character_data[character]
     ...
     core:add_listener(
          ...
          function(context)
               ...
               return faction_key == faction_name and
               self:is_match_key_from_list(building_key, character_info.required_buildings)
          end,
          function(context)
               local faction_key = context:building():faction():name()
               cm:trigger_mission(faction_key, self:get_mission_key(character_info.starting_mission_keys, faction_key),
                    true)
...
cm:add_loading_game_callback(
     function(context)
          if cm:is_new_game() == false then
               for i = 1, #character_unlocking.character_list do
                    local data = character_unlocking.character_data[character_unlocking.character_list[i]]
                    data.factions_involved = cm:load_named_value(data.name .. ".factions_involved",
                         data.factions_involved, context)
                    data.has_spawned = cm:load_named_value(data.name .. ".has_spawned", data.has_spawned, context)
                    if data.allowed_factions then
                         data.allowed_factions = cm:load_named_value(data.name .. ".allowed_factions",
                              data.allowed_factions, context)
                    end
               end
          end
     end
