...
local gotrek_subculture_details = {
     ...
     ["wh3_main_sc_ksl_kislev"] = { "wh3_main_ksl_growth_recruit_cost_2", "wh3_main_ksl_city_recruit_growth_xp_2" },
...
function gotrek_setup()
               ...
               if buildings then
                    for i = 1, #buildings do
                         if buildings[i] == building:name() then
                              ...
                              faction:name(), building:region():name(), false, true, 9)
     ...
     core:add_listener(
          ...
          function(context)
               ...
               if context:choice() == 0 then
                    ...
                    if character then
                         local pos_x, pos_y = cm:find_valid_spawn_location_for_character_from_character(faction_name,
                              cm:char_lookup_str(character), true, 2)

                         if pos_x > 1 then
                              local function spawn_gotrek_and_felix_post_dilemma(intervention)
                                   -- Spawn Gotrek
                                   cm:spawn_unique_agent_at_character(faction:command_queue_index(),
                                        "wh3_dlc25_neu_gotrek_hero", gotrek_details.spawn_cqi, true)
              cm:spawn_unique_agent_at_character(faction:command_queue_index(),
                                        "wh2_pro08_neu_felix", gotrek_details.spawn_cqi, true)
                                   ...
                                   "wh2_pro08_gotrek_felix_cooldown")
                                   effect_bundle:set_duration(gotrek_turns_available)
                                   incident_payload:effect_bundle_to_faction(effect_bundle)
                                   cm:trigger_custom_incident(faction_name, "wh2_pro08_incident_neu_unlocking", true,
                                        incident_payload)

                                   trigger_gotrek_and_felix_cutscene("gotrek_felix_arrival", gotrek_details.spawn_cqi,
                                        { pos_x, pos_y }, false, intervention)
                              end
                              ...
                              if cm:is_multiplayer() then
                                   ...
                                   cm:trigger_transient_intervention(
                                        ...
                                        function(intervention)
                                             ...
                                             "faction_event_incidentevent_feed_target_incident_faction")
                                             intervention:whitelist_events(
                                             "faction_event_character_incidentevent_feed_target_incident_faction")
                                             intervention:whitelist_events(
                                             "faction_event_region_incidentevent_feed_target_incident_faction")
                                        end
                                   )
                              end
     ...
     core:add_listener(
          ...
          function(context)
               ...
               return character:character_subtype("wh3_dlc25_neu_gotrek_hero") or
               character:character_subtype("wh2_pro08_neu_felix")
          end,
          function(context)
               local character = context:character()
               ...
               if character:character_subtype("wh3_dlc25_neu_gotrek_hero") then
                    -- Gotrek has spawned
     ...
     core:add_listener(
          ...
          function(context)
               ...
               (gotrek_details.state == gotrek_state.spawned or gotrek_details.state == gotrek_state.spawned_ai) and
               gotrek_details.cooldown > 0
          end,
          function(context)
     ...
     core:add_listener(
          ...
          function()
               if gotrek_details.owner then
                    ...
                    return gotrek_details.state == gotrek_state.spawned_ai and owner and
                    (owner:is_null_interface() or owner:is_dead())
               end
          end,
          function()
               kill_gotrek_and_felix_characters()
     ...
     core:add_listener(
          ...
          "WorldStartRound",
          function()
               return gotrek_details.cooldown and gotrek_details.cooldown > 0 and gotrek_details.state and
               (gotrek_details.state == gotrek_state.cooldown or gotrek_details.state == gotrek_state.cooldown_ai)
          end,
          function()
               gotrek_details.cooldown = gotrek_details.cooldown - 1
               ...
               if gotrek_details.cooldown == 0 then
                    if gotrek_details.state == gotrek_state.cooldown then
                         ...
                         if faction:has_home_region() then
                              ...
                              local pos_x, pos_y = cm:find_valid_spawn_location_for_character_from_settlement(
                              gotrek_details.owner, region_key, false, true, 3)
                              ...
                              if pos_x > 1 then
                                   ...
                                   cm:create_force_with_general(
                                        ...
                                        function(cqi)
                                             ...
                                             -- Spawn Felix
                                             cm:spawn_unique_agent_at_character(faction:command_queue_index(),
                                                  "wh2_pro08_neu_felix", gotrek_details.gotrek_cqi, true)
                                        end
                                   )
                    ...
                    elseif gotrek_details.state == gotrek_state.cooldown_ai then
                         ...
                         if #possible_regions > 0 then
                              ...
                              local pos_x, pos_y = cm:find_valid_spawn_location_for_character_from_settlement(
                              faction:name(), region:name(), false, true, 9)
...
function kill_gotrek_and_felix_characters()
     ...
     if gotrek_char and not gotrek_char:is_null_interface() and gotrek_char:character_subtype("wh3_dlc25_neu_gotrek_hero") then
          -- Kill Gotrek
