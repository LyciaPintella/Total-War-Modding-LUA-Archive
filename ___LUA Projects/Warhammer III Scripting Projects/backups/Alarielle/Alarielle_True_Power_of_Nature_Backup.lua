        ...
        if character:has_region() and not character:is_at_sea() then
            ...
            if not region:is_abandoned() then
                ...
                if power_of_nature["cultures"][region_culture] ~= nil then
                    ...
                    -- out("Same faction?: " .. tostring(region_owning_faction:name() == character:faction():name()) .. "")
                    -- or region_owning_faction:name() == character:faction():name() then
...
function defender_of_ulthuan_show_event(region, event_type)
    if event_type == "united" then
        cm:show_message_event(alarielle_faction_key, "event_feed_strings_text_wh2_dlc10_event_feed_string_scripted_event_defender_of_ulthuan_title",
        ...
        cm:show_message_event_located(alarielle_faction_key, "event_feed_strings_text_" .. "wh2_dlc10_event_feed_string_scripted_event_defender_of_ulthuan_title",
