...
local function JADLOG(text)
     out("JADLOG(text): JadLogFile_Made is " .. tostring(JadLogFile_Made) ..
     ...
     if popLog then
          popLog:write(
               "Dynamic Difficulty: " .. logText .. " [" .. logTimeStamp .. "] \n")
          popLog:flush()
     else
...
local function read_mct_values(ignore_setting_lock)
     ...
     if mct_jdyndif then
          local mct_mymod = mct_jdyndif:get_mod_by_key(
               "jadawin_dynamic_difficulty")
          if ignore_setting_lock or
              not (mct_mymod:get_option_by_key("settings_locked")
                   :get_finalized_setting()) then
               cm:set_saved_value("jdyndif_enable_replenishment",
                    mct_mymod:get_option_by_key(
                         "enable_replenishment")
                    :get_finalized_setting())
               cm:set_saved_value("jdyndif_enable_recruitment",
                    mct_mymod:get_option_by_key("enable_recruitment")
                    :get_finalized_setting())
               cm:set_saved_value("jdyndif_extra_difficulty",
                    mct_mymod:get_option_by_key("extra_difficulty")
                    :get_finalized_setting())
...
local function get_empire_score_player()
     ...
     elseif human_faction_key == "wh2_dlc13_lzd_spirits_of_the_jungle" then
          ...
          local vassal_regions = cm:model():world():faction_by_key(
     ...
     elseif subculture == "wh_main_sc_nor_norsca" then
          ...
          for i = 0, owned_regions:num_items() - 1 do
               ...
               if region:province_name() == "wh3_main_combi_province_albion" or
                   region:province_name() ==
                   "wh3_main_combi_province_vanaheim_mountains" or
                   region:province_name() ==
                   "wh3_main_combi_province_helspire_mountains" or
                   region:province_name() ==
                   "wh3_main_combi_province_ice_tooth_mountains" or
                   region:province_name() ==
                   "wh3_main_combi_province_mountains_of_naglfari" or
                   region:province_name() ==
                   "wh3_main_combi_province_trollheim_mountains" or
                   region:province_name() ==
                   "wh3_main_combi_province_mountains_of_hel" or
                   region:province_name() ==
                   "wh3_main_combi_province_gianthome_mountains" or
                   region:province_name() ==
     ...
     elseif subculture == "wh_dlc05_sc_wef_wood_elves" then
          ...
          for i = 0, owned_regions:num_items() - 1 do
               local region = owned_regions:item_at(i)
               if region:name() == "wh3_main_combi_region_kings_glade" or
                   region:name() == "wh3_main_combi_region_vauls_anvil_loren" or
                   region:name() == "wh3_main_combi_region_waterfall_palace" or
                   region:name() == "wh3_main_combi_region_crag_halls_of_findol" or
                   region:name() == "wh3_main_combi_region_the_witchwood" or
                   region:name() == "wh3_main_combi_region_gaean_vale" or
                   region:name() == "wh3_main_combi_region_laurelorn_forest" or
                   region:name() == "wh3_main_combi_region_gryphon_wood" or
                   region:name() == "wh3_main_combi_region_forest_of_gloom" or
                   region:name() == "wh3_main_combi_region_oreons_camp" or
                   region:name() == "wh3_main_combi_region_the_haunted_forest" or
                   region:name() == "wh3_main_combi_region_jungles_of_chian" or
     ...
     elseif subculture == "wh_main_sc_chs_chaos" then
          ...
          for i = 0, factions:num_items() - 1 do
               if factions:item_at(i):is_vassal_of(human_faction) then
                    vassals_region_count = vassals_region_count +
                        factions:item_at(i):region_list()
                        :num_items()
               end
          end
...
core:add_listener("MCT_JDYNDIF", "MctInitialized", true,
     function(context) mct_jdyndif = context:mct() end, true)

-- Listener for when the MCT settings screen was opened and settings changed
core:add_listener("MCT_CHANGED_JDYNDIF", "MctFinalized", true,
     function(context)
...
core:add_listener("MCT_PANEL_OPENED_JDYNDIF", "MctPanelOpened", true,
     function(context)
          ...
          if mct_mymod:get_option_by_key("settings_locked"):get_finalized_setting() then
               mct_mymod:get_option_by_key("jdyndif_enable_replenishment")
                   :set_read_only(true)
               mct_mymod:get_option_by_key("jdyndif_enable_recruitment"):set_read_only(
                    true)
          end
     end, true)

-- event player starts turn
core:add_listener("JDYNDIF_TURNSTART", "FactionTurnStart",
     function(context) return (context:faction():is_human()) end,
     function(context)
          local turn_number = cm:model():turn_number()
          cm:callback(function()
               if turn_number == 1 or
                   not (cm:get_saved_value("jdyndif_mct_values_exist")) then
...
core:add_listener("JDYNDIF_EFFECT", "FactionTurnStart", function(context)
     ...
     local human_faction_key = (cm:get_human_factions())[1]
     local human_faction = ((cm:model()):world()):faction_by_key(
          human_faction_key)
     ...
     if current_faction_name ~= "rebels" then
          cm:callback(function()
               ...
               if current_faction:is_vassal_of(human_faction) == true or
                   current_faction:allied_with(human_faction) == true then
               ...
               if cm:get_saved_value("jdyndif_extra_difficulty") > 0 then
                    mod_extra_difficulty = cm:get_saved_value(
                             "jdyndif_extra_difficulty") * 0.05 +
                        1
               end

               local effect_strength_tax_rate =
                   math.ceil(ai_buff_level * 4 * mod_tax_rate *
                        mod_extra_difficulty)
               if effect_strength_tax_rate > 175 then
                    effect_strength_tax_rate = 175
               end

               local effect_strength_battle_loot =
                   math.ceil(ai_buff_level * 4 * mod_battle_loot *
                        mod_extra_difficulty)
               if effect_strength_battle_loot > 175 then
                    effect_strength_battle_loot = 175
               end

               local effect_strength_growth =
                   math.ceil(ai_buff_level * 4 * mod_growth * mod_extra_difficulty)
               if effect_strength_growth > 75 then
                    effect_strength_growth = 75
               end

               local effect_strength_construction = math.ceil(
                    ai_buff_level * -4 *
                    mod_construction *
                    mod_extra_difficulty)
               ...
               local effect_strength_recruit_cost = math.ceil(
                    ai_buff_level * -4 *
                    mod_recruit_cost *
                    mod_extra_difficulty)
               ...
               local Dynamic_Difficulty_Government =
                   cm:create_new_custom_effect_bundle("effect_bundle_government")

               Dynamic_Difficulty_Government:add_effect(
                    "wh_main_effect_economy_gdp_mod_all", "faction_to_region_own",
                    effect_strength_tax_rate)
               Dynamic_Difficulty_Government:add_effect(
                    "wh_main_effect_building_construction_cost_mod",
                    "faction_to_region_own", effect_strength_construction)
               Dynamic_Difficulty_Government:add_effect(
                    "wh_main_effect_province_growth_other",
               ...
               cm:apply_custom_effect_bundle_to_faction(
                    Dynamic_Difficulty_Government, current_faction)

               local Dynamic_Difficulty_Armies =
                   cm:create_new_custom_effect_bundle("effect_bundle_armies")
               ...
               Dynamic_Difficulty_Armies:add_effect(
                    "wh_main_effect_agent_action_outcome_parent_army_xp_gain_factionwide",
                    "faction_to_force_own", xp_gain_per_turn)
               Dynamic_Difficulty_Armies:add_effect(
               ...
               if cm:get_saved_value("jdyndif_enable_replenishment") then
                    Dynamic_Difficulty_Armies:add_effect(
                         "wh2_main_effect_replenishment_characters",
               ...
               cm:apply_custom_effect_bundle_to_faction(Dynamic_Difficulty_Armies,
                    current_faction)

               -- out("Checking current AI faction turn to trigger logging" .. "")
               if current_faction_name == "wh_dlc07_vmp_von_carstein" or
                   current_faction_name == "wh2_main_def_naggarond" or
                   current_faction_name == "wh2_main_def_har_ganeth" then
                    jlog("Turn: # " .. turn_number .. " |  Difficulty: " ..
                         player_difficulty .. " | Player Score: " ..
                         player_score .. "| AI Buff Level: " .. ai_buff_level ..
                         " | Income: " .. "+" .. effect_strength_tax_rate .. "%" ..
                         " | Growth: " .. "+" .. effect_strength_growth ..
                         " | Construction Cost: " ..
                         effect_strength_construction .. "%" ..
                         " | Recruit Cost: " .. effect_strength_recruit_cost ..
                         "%" .. " | Loot: " .. "+" ..
                         effect_strength_battle_loot .. "%" .. " | Rank: " ..
                         "+" .. recruit_rank .. " | Unit XP Per Turn: " .. "+" ..
                         xp_gain_per_turn .. " | Replenishment: " .. "+" ..
                         replenishment_bonus .. "% | Recruitment Slots: " .. "+" ..
                         recruit_points_bonus)
               end
          end, 0)
     end
end, true)
