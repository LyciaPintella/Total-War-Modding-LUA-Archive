...
local gotrek_subculture_details = {
    ...
    ["wh3_main_sc_ksl_kislev"] = {"wh3_main_ksl_growth_recruit_cost_1"},
    ["wh2_main_sc_hef_high_elves"] = {"wh2_main_hef_order_1"},
    ["wh_dlc05_sc_wef_wood_elves"] = {"wh_dlc05_wef_public_order_1"},
    ["wh2_main_sc_lzd_lizardmen"] = {"wh2_main_lzd_order_1"},
    ["wh2_dlc09_sc_tmb_tomb_kings"] = {"wh2_dlc09_tmb_public_order_1"},
    ["wh3_main_sc_cth_cathay"] = {"wh3_main_cth_growth_yin_1"}
}
...
function gotrek_setup()
    -- Building Completed
    core:add_listener("gotrek_BuildingCompleted", "BuildingCompleted", function(context)
                ...
                if buildings[i] == building:name() then
                    ...
                    if pos_x > -1 then
                        ...
                        local function show_gotrek_and_felix_appear_event(event_faction, x, y)
                            cm:show_message_event_located(event_faction:name(), "event_feed_strings_text_wh2_pro08_event_feed_string_scripted_event_gotrek_felix_title",
                                                          "event_feed_strings_text_wh2_pro08_event_feed_string_scripted_event_gotrek_felix_appear_primary_detail",
                                                          "event_feed_strings_text_wh2_pro08_event_feed_string_scripted_event_gotrek_felix_appear_secondary_detail", x, y, false, 1308)
                        end
    ...
    -- Marker Entered
    core:add_listener("gotrek_AreaEntered", "AreaEntered", function(context)
    ...
    -- Dilemma Choice
    core:add_listener("gotrek_DilemmaChoiceMadeEvent", "DilemmaChoiceMadeEvent", function(context)
    ...
    core:add_listener("gotrek_FactionBeginTurnPhaseNormal", "FactionBeginTurnPhaseNormal", function(context)
        return gotrek_details.owner and context:faction():name() == gotrek_details.owner and (gotrek_details.state == gotrek_state.spawned or gotrek_details.state == gotrek_state.spawned_ai) and
                   gotrek_details.cooldown > 0
    end, function(context)
        ...
        if gotrek_details.cooldown == 0 then
            ...
            if cm:is_multiplayer() then
                ...
                cm:trigger_transient_intervention("g_f_leave_faction_turn_start", function(intervention)
                    ...
                    if pos_x > -1 then
                        ...
                        local function show_gotrek_and_felix_reappear_event(event_faction, x, y)
                            ...
                            cm:show_message_event_located(event_faction:name(), "event_feed_strings_text_wh2_pro08_event_feed_string_scripted_event_gotrek_felix_title",
                                                          ...
                                                          "event_feed_strings_text_wh2_pro08_event_feed_string_scripted_event_gotrek_felix_reappear_" .. event_apx .. "_secondary_detail", x, y, false,
                                                          1308)
                        end
...
function trigger_gotrek_and_felix_cutscene(key, cqi, loc, kill, intervention)
    ...
    gotrek_and_felix_cutscene:set_skippable(true, function()
        gotrek_and_felix_cutscene_skipped(kill)
    end)
    ...
    core:add_listener("skip_camera_after_vo_ended", "ScriptTriggeredVOFinished", true, function()
...
function gotrek_and_felix_cutscene_skipped(kill)
    ...
    cm:callback(function()
        cm:override_ui("disable_advice_audio", false)
    end, 0.5)
    cm:restore_shroud_from_snapshot()
    if kill then
        kill_gotrek_and_felix_characters()
