...
local function JADSESSIONLOG()
    ...
    if not JadLogFile_Made then
        out("#### Dynamic Difficulty line 15: popLog is " .. tostring(popLog) .. " JadLogFile_Made is " .. tostring(JadLogFile_Made) .. " ####")
    else
        out("#### Dynamic Difficulty line 17: popLog is " .. tostring(popLog) .. " JadLogFile_Made is " .. tostring(JadLogFile_Made) .. " ####")
    end
...
local function JADLOG(text)
    out("#### Dynamic Difficulty line 42: JadLogFile_Made is " .. tostring(JadLogFile_Made) .. " . JadLogging_Enabled is " .. tostring(JadLogging_Enabled) .. " ####")
    if not JadLogging_Enabled then
        return
    end
    ...
    out("#### Dynamic Difficulty line 51: Jadlog_Filename is " .. tostring(Jadlog_Filename) .. " . popLog is " .. tostring(popLog) .. " ####")
...
local function get_empire_score_player()
    ...
    elseif (subculture == "wh_main_sc_nor_norsca") then
        ...
        for i = 0, owned_regions:num_items() - 1 do
            ...
            if (region:province_name() == "wh3_main_combi_province_albion" or region:province_name() == "wh3_main_combi_province_vanaheim_mountains" or region:province_name() ==
                "wh3_main_combi_province_helspire_mountains" or region:province_name() == "wh3_main_combi_province_ice_tooth_mountains" or region:province_name() ==
                "wh3_main_combi_province_mountains_of_naglfari" or region:province_name() == "wh3_main_combi_province_trollheim_mountains" or region:province_name() ==
                "wh3_main_combi_province_mountains_of_hel" or region:province_name() == "wh3_main_combi_province_gianthome_mountains" or region:province_name() ==
                "wh3_main_combi_province_goromadny_mountains") then
    ...
    elseif (subculture == "wh_dlc05_sc_wef_wood_elves") then
        ...
        for i = 0, owned_regions:num_items() - 1 do
            local region = owned_regions:item_at(i)
            if (region:name() == "wh3_main_combi_region_kings_glade" or region:name() == "wh3_main_combi_region_vauls_anvil_loren" or region:name() == "wh3_main_combi_region_waterfall_palace" or
                region:name() == "wh3_main_combi_region_crag_halls_of_findol" or region:name() == "wh3_main_combi_region_the_witchwood" or region:name() == "wh3_main_combi_region_gaean_vale" or
                region:name() == "wh3_main_combi_region_laurelorn_forest" or region:name() == "wh3_main_combi_region_gryphon_wood" or region:name() == "wh3_main_combi_region_forest_of_gloom" or
                region:name() == "wh3_main_combi_region_oreons_camp" or region:name() == "wh3_main_combi_region_the_haunted_forest" or region:name() == "wh3_main_combi_region_jungles_of_chian" or
...
-- add event
core:add_listener("JDYNDIF_EFFECT", "FactionTurnStart", -- function(context) return (true) end, --debug
function(context)
    ...
    if current_faction_name ~= "rebels" then
        cm:callback(function()
            ...
            if combined_difficulty == -1 then -- Hard
                ...
                mod_construction = 1.2
                mod_recruit_cost = 1.2
                mod_tax_rate = 1.2
                mod_growth = 1.2
                mod_battle_loot = 1.2
                mod_upkeep = 1.1
            elseif combined_difficulty == -2 then -- Very Hard
                ...
                mod_construction = 1.3
                mod_recruit_cost = 1.3
                mod_tax_rate = 1.3
                mod_growth = 1.3
                mod_battle_loot = 1.3
                mod_upkeep = 1.15
            ...
            elseif ai_buff_level >= 6 and combined_difficulty < 0 then
                ...
                recruit_rank = math.ceil(1.5 * (math.abs(combined_difficulty)))
            ...
            if cm:get_saved_value("jdyndif_extra_difficulty") > 0 then
                mod_extra_difficulty = (cm:get_saved_value("jdyndif_extra_difficulty") * 0.1) + 1
            end

            local effect_strength_tax_rate = math.ceil(ai_buff_level * 4 * mod_tax_rate * mod_extra_difficulty + 3)
            if effect_strength_tax_rate > 245 then
                effect_strength_tax_rate = 245
            end
            local effect_strength_upkeep = math.ceil(ai_buff_level * -2.25 * mod_upkeep * mod_extra_difficulty)
            if effect_strength_upkeep < -45 then
                effect_strength_upkeep = -45
            end
            ...
            local effect_strength_growth = math.ceil(ai_buff_level * 4 * mod_growth * mod_extra_difficulty + 7)
            if effect_strength_growth > 125 then
                effect_strength_growth = 125
            end
            local effect_strength_construction = math.ceil(ai_buff_level * -2.25 * mod_construction * mod_extra_difficulty - 5)
            if effect_strength_construction < -75 then
                effect_strength_construction = -75
            end

            local effect_bundle = cm:create_new_custom_effect_bundle("jadawin_ai_difficulty")
            effect_bundle:add_effect("wh_main_effect_economy_gdp_mod_all", "faction_to_region_own", effect_strength_tax_rate)
            effect_bundle:add_effect("wh_main_effect_building_construction_cost_mod", "faction_to_region_own", effect_strength_construction)
            effect_bundle:add_effect("wh_main_effect_province_growth_other", "faction_to_province_own", effect_strength_growth)
            effect_bundle:add_effect("wh_main_effect_agent_action_outcome_parent_army_xp_gain_factionwide", "faction_to_force_own", xp_gain_per_turn)
            effect_bundle:add_effect("wh_main_effect_force_all_campaign_experience_base_all", "faction_to_force_own", recruit_rank)
            effect_bundle:add_effect("wh_main_effect_force_all_campaign_post_battle_loot_mod", "faction_to_faction_own", effect_strength_battle_loot)
            if (cm:get_saved_value("jdyndif_enable_replenishment")) then
                effect_bundle:add_effect("wh2_main_effect_replenishment_characters", "faction_to_force_own", replenishment_bonus)
                effect_bundle:add_effect("wh_main_effect_force_all_campaign_replenishment_rate", "faction_to_force_own", replenishment_bonus)
            end
            if (cm:get_saved_value("jdyndif_enable_recruitment")) then
                effect_bundle:add_effect("wh_main_effect_unit_recruitment_points", "faction_to_province_own", recruit_points_bonus)
            end
            effect_bundle:set_duration(1)
            cm:apply_custom_effect_bundle_to_faction(effect_bundle, current_faction)

            if (current_faction_name == "wh_main_emp_empire") or (current_faction_name == "wh_dlc07_vmp_von_carstein") then
                jlog("Turn: # " .. turn_number .. " |  Difficulty: " .. player_difficulty .. " | Player Score: " .. player_score .. "| AI Faction: " .. tostring(current_faction_name) ..
                         " | AI Buff Level: " .. ai_buff_level .. " | Income: " .. "+" .. effect_strength_tax_rate .. "%" .. " | Growth: " .. "+" .. effect_strength_growth .. " | Construction Cost: " ..
                         effect_strength_construction .. "%" .. " | Loot: " .. "+" .. effect_strength_battle_loot .. "%" .. " | Rank: " .. "+" .. recruit_rank .. " | Unit XP Per Turn: " .. "+" ..
