-- Examples :
-- lvl 10 with no magic find : 90 weight for common, 32 for uncommon, 12 for rare.
-- lvl 25 with 10% magic find : 73 weight for common, 40 for uncommon, 20 for rare.
-- lvl 50 with 50% magic find : 30 weight for common, 50 for uncommon, 30 for rare.
local drop_rarities = {
     ["common"] = {
          ["base"] = 100,
          ["per_level"] = -0.75,
...
-- The system is only applied to human players. Bots have a simpler system, which is faster. The difference is quite unnoticeable, but even a few milliseconds off the end turn times are good to take I guess.
-- Example : rare 90, uncommon, 61 => the bot need  to roll a dice 100 above 90 to have a rare item. If it's below but above 61, then it will drop an uncommon one. Else a common one.
-- The per_level gives a bonus times the rank of the character to the dice roll.
local drop_rarity_AI = {
     ["rare"] = 85,
     ["uncommon"] = 55,
     ["per_level"] = 0.4
}

-- Unless you want the mod to scale with some crazy level caps or magic find values, you don't have to tweak this.
local caps = {
     ["max_level"] = 65,
     ["max_magic_find"] = 100
}

-- In the base game, tomb kings have reduced drop rate to account for the crafting mechanics.
-- You can add other factions cultures with a value between 0 and 1.
local culture_chance_modifiers = {
     ["wh2_dlc09_tmb_tomb_kings"] = 0.5
}
...
core:add_listener(
     ...
     function(context)
          ...
          return character:won_battle() and cm:char_is_general_with_army(character) and
              not context:has_stolen_ancillary() and attempt_to_award_random_magical_item(context);
     end,
     true
);
...
function attempt_to_award_random_magical_item(context)
     if not cm:model():campaign_name("wh3_main_prologue") then
          ...
          if (attacker and attacker:is_quest_battle_faction()) or (defender and defender:is_quest_battle_faction()) then
               out.traits(
                    "Agarast Drop Chance Tweaker: attempt_to_award_random_magical_item() called, but it is a quest battle. Not going to award anything.");
               return;
          end;
          ...
          if character:is_caster() or cm:general_has_caster_embedded_in_army(character) then
               index = cm:random_number(6); -- this will weigh slightly towards arcane items when the character is a caster (6 or higher)
          else
               index = cm:random_number(6); -- don't drop arcane items if the character involved is not a caster
          end;
          ...
          if (culture_chance_modifiers[faction:culture()] ~= nil) then
               chance = chance * culture_chance_modifiers[faction:culture()];
          end

          -- SELECT ANCILLARY RARITY

          -- get the list of ancillaries based on the rarity
          local rank = character:rank();
          if (rank > caps.max_level) then rank = caps.max_level end;

          if (faction:is_human()) then
               if (bv_chance > caps.max_magic_find) then bv_chance = caps.max_magic_find end;

               local rarities = weighted_list:new();
               for rarity, rarity_table in pairs(drop_rarities) do
                    local current_rarity_weight = math.floor(rarity_table.base + (rank * rarity_table.per_level) +
                         (bv_chance * rarity_table.per_magic_find));
                    rarities:add_item(new_ancillary_list[rarity], current_rarity_weight);
               end
               new_ancillary_list, index_anc = rarities:weighted_select();
          else
               local rarity_roll = cm:random_number(100) + (drop_rarity_AI.per_level * rank);
               if rarity_roll > drop_rarity_AI.rare then
                    new_ancillary_list = new_ancillary_list.rare;
               elseif rarity_roll > drop_rarity_AI.uncommon then
          ...
          out.traits("Agarast Drop Chance Tweaker: Rolled a " ..
               roll ..
               " to assign an ancillary with a chance of " ..
               chance .. " for a character belonging to the faction " .. faction:name());
          ...
          if roll <= chance then
               ...
               while not can_equip and count < 20 do
                    ...
                    if character:can_equip_ancillary(chosen_ancillary) then
                         can_equip = true;
                         out.traits("Agarast Drop Chance Tweaker: Trying to assign the ancillary " ..
                              chosen_ancillary .. " for a character belonging to the faction " .. faction:name());
...
function get_random_ancillary_key_for_faction(faction_key, specified_category, rarity)
     ...
     if not faction then
          script_error("ERROR: get_random_ancillary_key_for_faction() called but no faction with supplied key [" ..
     ...
     if not faction:has_faction_leader() then
          script_error("ERROR: get_random_ancillary_key_for_faction() called but faction with supplied key [" ..
               faction_key .. "] has no faction leader - how can this be?");
          return;
     end;
     ...
     if not ancillary_list[category] then
          script_error("ERROR: get_random_ancillary_key_for_faction() called with category [" ..
               category ..
               "], but this is not a valid ancillary category. Valid categories are " ..
               table.concat(valid_ancillary_categories, ", "));
          return;
     end;
     ...
     if not ancillary_list[category][rarity] then
          script_error("Agarast Drop Chance Tweaker: ERROR: get_random_ancillary_key_for_faction() called with rarity [" ..
               rarity ..
               "], but this is not a valid ancillary rarity. Valid rarities are " ..
               table.concat(valid_ancillary_rarities, ", "));
          return;
     end;
